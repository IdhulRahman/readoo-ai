...
const ANIMATION_MAP = {
    ...
    wave: '/assets/animations/wave.vrma',
    thinking: '/assets/animations/thinking.vrma'
};
...
async function loadVrmaAnimation(url, name) {
    const loader = new GLTFLoader();
    ...
    try {
        ...
        console.warn(`Failed to load animation: ${name} from ${url}. Falling back to idle.`);
        return loadedClips.idle;
    }
}

async function loadAllVrmaAnimations() {
    try {
        await loadVrmaAnimation(ANIMATION_MAP.idle, 'idle');
        playAnimation('idle');
        updateUiStatus("Siap");

        // Preload other animations in background
        loadVrmaAnimation(ANIMATION_MAP.wave, 'wave').catch(() => {});
        loadVrmaAnimation(ANIMATION_MAP.thinking, 'thinking').catch(() => {});
    } catch {}
}
...
function playAnimation(name) {
    ...
    const newClip = loadedClips[name] || loadedClips.idle;
